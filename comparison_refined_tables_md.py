...
from utils.metric_md import TEDS
...
def calculate_std_similarity_html2md(text1, text2):
    ...
    # Calculate both regular and structure-ony similarity scores
...
def extract_text(sentence):
    ...
    Args:
        sentence (str): A string that looks like a JSON object containing natural_text field
            Example input: '{"natural_text":"| Column1 | Column2 |\\n|---------|----------|"}'
        
    Returns:
        str: The extracted markdown table text
    """
    try:
        # If the input is a string that looks like JSON, try to parse it
        if isinstance(sentence, str) and sentence.strip().startswith('{'):
            import json
            json_obj = json.loads(sentence)
            if "natural_text" in json_obj:
                return json_obj["natural_text"]
        
        # If not JSON or parsing fails, return the original string
        return str(sentence)
...
def compare_tables(gt_file, extracted_file):
    ...
    for i, gt in enumerate(gt_tables_norm, 1):
        ...
        for j, (orig_idx, ext) in enumerate(tables_to_compare, 1):
            ...
            if not "natural_text" in sentence: # that means the table is in html format
                sentence = ext['original']['original_sentence']
                sim, struct_sim = calculate_std_similarity(gt['original']['sentence'], sentence)
            else:
                md_sentence = extract_text(sentence)
                # print(md_sentence)
                sim, struct_sim = calculate_std_similarity_html2md(gt['original']['sentence'], md_sentence)
...
def process_folders(mineru_folder: str, 
    ...
    mineru_files = {f.replace('.tables.refined.json', '.pages.tables.json'): f 
                   for f in os.listdir(mineru_folder) if f.endswith('.tables.refined.json')}
...
def html2markdown(html_str: str) -> str:
    """Convert HTML table to markdown format."""
    try:
        return htmltabletomd.convert_table(html_str)
    except Exception as e:
