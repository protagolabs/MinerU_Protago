...
def markdown_table_to_html(markdown_table: str) -> str:
    ...
    html_parts.extend(f'<th>{cell}</th>' for cell in header_cells)
    html_parts.append('</tr>')
...
class TEDS(object):
    ...
    def evaluate(self, pred, true):
        ''' Computes TEDS score between the prediction and the ground truth of a
            given sample. Now handles both HTML and markdown input.
        '''
        if (not pred) or (not true):
            return 0.0
        
        # Convert markdown to HTML if input appears to be markdown
        if pred.strip().startswith('|') and pred.strip().endswith('|'):
            pred = markdown_table_to_html(pred)
        if true.strip().startswith('|') and true.strip().endswith('|'):
            true = markdown_table_to_html(true)
        
        # Wrap in HTML body if not already wrapped
        if not pred.strip().startswith('<html>'):
            pred = f'<html><body>{pred}</body></html>'
        if not true.strip().startswith('<html>'):
            true = f'<html><body>{true}</body></html>'
