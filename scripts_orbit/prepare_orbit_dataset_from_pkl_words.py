    ...
    for table_data in tables:
        ...
        if not source_image_path:
            ...
            for cell in table_data.get("cells", []):
                ...
                if words:
                    # Combine all words into a single token for matching
                    combined_token = " ".join([word.get("content", "") for word in words if word.get("content", "")])
                    
                    # Get words with bounding boxes
                    words_with_bbox = [word for word in words if "word_bbox" in word]
                    
                    if words_with_bbox and combined_token:
                        # Compute union of all word bounding boxes
                        x1 = min(int(word["word_bbox"][0]) for word in words_with_bbox)
                        y1 = min(int(word["word_bbox"][1]) for word in words_with_bbox)
                        x2 = max(int(word["word_bbox"][2]) for word in words_with_bbox)
                        y2 = max(int(word["word_bbox"][3]) for word in words_with_bbox)
                        bbox = [[x1, y1], [x2, y1], [x2, y2], [x1, y2]]
                        
                        # Create a cell_data with the combined token and union bbox
                        cell_data = {
                            "tokens": [list(combined_token)],
                            "bbox": [bbox]
                        }
                        html_cells.append(cell_data)
                    else:
                        # Add empty cell if no valid word with bbox found
                        html_cells.append({"tokens": [], "bbox": []})
                else:
                    # Fallback to cell content if no words are available
                    if content and "bounding_regions" in cell and cell["bounding_regions"]:
                        ...
                        if "cell_bbox" in bounding_region:
                            ...
                            # Create a cell_data with the cell content
                            cell_data = {
                                "tokens": [content],
                                "bbox": [bbox]
                            }
