...
def extract_tables_to_json(data: Any, output_dir: str, filename: str, pdf_path: str = None, visualize: bool = False) -> List[Dict]:
    ...
    Args:
        ...
        visualize: Whether to create visualizations with bounding boxes
    ...
    if hasattr(data, 'tables') and data.tables:
        for table_idx, table in enumerate(data.tables):
            ...
            if hasattr(table, 'cells') and table.cells:
                ...
                for cell in table.cells:
                    ...
                    if hasattr(cell, 'spans') and cell.spans:
                        ...
                        for span in cell.spans:
                            ...
                            cell_data["spans"].append(span_data)
                ...
                # Create visualization of the table with cell bounding boxes if requested
                if visualize and pdf_doc and "image_paths" in table_data:
                    try:
                        for img_path in table_data["image_paths"]:
                            ...
                            if os.path.exists(full_img_path):
                                ...
                                for cell_idx, cell in enumerate(table_data["cells"]):
                                    if "bounding_regions" in cell:
                                        for region in cell["bounding_regions"]:
                                            if "polygon_pixels" in region:
                                                # Get polygon points
                                                polygon = region["polygon_pixels"]
                                                
                                                # Convert to image coordinates
                                                img_polygon = []
                                                for point in polygon:
                                                    # Adjust coordinates relative to table bbox
                                                    if "bbox" in table_data:
                                                        x = point["x"] - table_data["bbox"][0]
                                                        y = point["y"] - table_data["bbox"][1]
                                                    else:
                                                        x = point["x"]
                                                        y = point["y"]
                                                    
                                                    # Scale to image dimensions
                                                    x = x * (img.width / (table_data["bbox"][2] - table_data["bbox"][0]))
                                                    y = y * (img.height / (table_data["bbox"][3] - table_data["bbox"][1]))
                                                    
                                                    img_polygon.append((x, y))
                                                ...
                                                if len(img_polygon) >= 3:  # Need at least 3 points for a polygon
                                                    draw.polygon(img_polygon, outline=color, width=2)
                                                    ...
                                                    # Draw cell content
                                                    # text = f"{row},{col}: {cell['content'][:20]}"
...
def main():
    ...
    parser.add_argument(
        "--visualize", 
        action="store_true",
        help="Create visualizations of cell bounding boxes on table images"
    )
