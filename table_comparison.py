...
def compare_tables(gt_file, extracted_file):
    ...
    for i, table in enumerate(gt_tables, 1):
        ...
        for j, extracted_table in enumerate(extracted_tables, 1):
            ...
            # print(similarity)
            # print(f"Similarity: {similarity:.4%}, Structure Similarity: {structure_similarity:.4%}")
        ...
        if best_match:
            ...
            detailed_matches.append({
                ...
                "mineru_text_preview": best_match_text_norm[:200] + "..." if len(best_match_text_norm) > 200 else best_match_text_norm,
                "azure_text_preview": gt_text_norm[:200] + "..." if gt_text_norm and len(gt_text_norm) > 200 else gt_text_norm
            })
...
def process_folders(mineru_folder: str, 
    ...
    # Get all JSON files
    mineru_files: Set[str] = {f for f in os.listdir(mineru_folder) if f.endswith('.json')}
    azure_files: Set[str] = {f for f in os.listdir(azure_folder) if f.endswith('.json')}
    ...
    # print(f"\nStarting comparison of {total_files} files...")

    # Process files with progress bar
    # for i, azure_file in enumerate(azure_files, 1):
    for azure_file in tqdm(azure_files, 
                        desc="tracking processed azure tables",
                        file=sys.stdout,  # Explicitly write to stdout
                        dynamic_ncols=True):  # Adapt to terminal width
        # print(f"\nProcessing file {i}/{total_files}: {azure_file}")
        ...
        if mineru_file in mineru_files:
            ...
            if results:
                ...
                total_avg_similarity += results['average_similarity']
...
def main() -> None:
    """Main function to run the table comparison tool."""
    try:
        args = parse_args()
