...
class ProtagoTableWrapper:
    ...
    def _get_ocr_lines_batch(self, images: List[PILImage.Image], languages: List[str]) -> List[List[dict]]:
        ...
        try:
            ...
            # Process each image with the OCR model
            all_table_cells = []
            for img, lang in zip(processed_images, languages):
                # Update language in OCR model if needed
                if lang is not None and hasattr(self.recognition_model, 'set_lang'):
                    self.recognition_model.set_lang(lang)
                
                # Run OCR on the image
                img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
                ocr_result = self.recognition_model.ocr(img, mfd_res=None, det=True, rec=True)
                
                # Convert OCR result to expected format
                table_cells = []
                if ocr_result and len(ocr_result) > 0:
                    # The OCR result is a list with one item containing all text regions
                    text_regions = ocr_result[0]
                    for region in text_regions:
                        # Each region is [polygon, (text, confidence)]
                        polygon = region[0]  # List of [x,y] points
                        text, confidence = region[1]  # Tuple of (text, confidence)
