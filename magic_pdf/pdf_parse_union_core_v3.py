"""
pdf_parse_union_core_v3.py - MULTITHREADED VERSION
==================================================
This is the multithreaded version of the PDF parsing core.
It processes multiple pages in parallel using ThreadPoolExecutor.
...
def pdf_parse_union(
    ...
    if len(img_crop_list) > 0:
        ...
        # Process OCR in batches for better performance
        batch_size = min(32, len(img_crop_list))  # Process in batches
        ocr_res_list = []
        
        for i in range(0, len(img_crop_list), batch_size):
            batch_images = img_crop_list[i:i + batch_size]
            batch_results = ocr_model.ocr(batch_images, det=False, tqdm_enable=False)[0]
            ocr_res_list.extend(batch_results)
    ...
    new_pdf_info_dict = {
        ...
        '_parse_type': 'v3_multithreaded',  # Version indicator
        '_version_name': 'pdf_parse_union_core_v3',
    }
